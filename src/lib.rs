...
fn phone(dir: &str, num: &str) -> String {
    let result = Regex::new(&format!(r"(?xm)^.*\+{}.*$", num))
        .unwrap()
        .find_iter(dir)
        .map(|mat| mat.as_str())
        .collect::<Vec<_>>();
    match result.len() {
        0 => return format!("Error => Not found: {}", num),
        1 => {
            let name = Regex::new(r".*<(.*)>.*")
                .unwrap()
                .captures(result.get(0).unwrap())
                .unwrap()
                .get(1)
                .unwrap()
                .as_str();
            let address = result
                ...
                .replace("  ", " ")
                .replace("_", " ");
            return format!("Phone => {}, Name => {}, Address => {}", num, name, address);
        }
        _ => return format!("Error => Too many people: {}", num),
    }
}
#[cfg(test)]
mod tests {
